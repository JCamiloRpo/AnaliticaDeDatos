# -*- coding: utf-8 -*-
"""Practica Vision Artificial.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1LBhMde6QI4l3N6uKVNxurP9y7nyqIk5h

# Juan Camilo Restrepo Velez 000373886
# Practica Vision Artificial - Reconocimiento de vehiculos y personas
"""
...
#Lectura de la carpeta de motos
path = "vehiculo"
...
for f in files:
  if f.endswith('.jpg'):
    ...
    nombres.append(f)
    img = Image.open('vehiculo/' + f) #Leer la imagen
    img = img.resize((500,500))
    img = img.convert('RGB')
    hist = img.histogram() #Calculamos histograma de color
    mat_hist.append(hist) #Adicianar el histograma
  
vehiculo = pd.DataFrame(mat_hist) #Data frame con los histogramas
...
for f in files:
  if f.endswith('.jpg'):
    print(f)
    nombres.append(f)
    img = Image.open('persona/' + f)
    img = img.resize((500,500))
    img = img.convert('RGB')
    hist = img.histogram()
    mat_hist.append(hist)
  
persona = pd.DataFrame(mat_hist) 
persona['clase'] = "persona"
persona

data = pd.concat([vehiculo, persona], axis=0)
data
...
data['clase'] = data['clase'].replace({'vehiculo':0, 'persona':1})
data
...
#Red neuronal
modelDL = Sequential()
modelDL.add(Dense(400, input_dim = 768, activation = 'relu'))
